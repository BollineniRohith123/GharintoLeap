...
const upload = (0, multer_1.default)({ storage: storage, limits: { fileSize: 10 * 1024 * 1024 } }); // 10MB limit
// Auth middleware
...
app.get('/users', authenticateToken, requirePermission('users.view'), async (req, res) => {
    try {
        ...
        if (roles && roles.length > 0) {
            for (const roleName of roles) {
                ...
                if (roleResult.rows.length > 0) {
                    await pool.query('INSERT INTO user_roles (user_id, role_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [newUser.id, roleResult.rows[0].id]);
                }
            }
        }
        res.status(201).json({
...
app.get('/users/:id', authenticateToken, requirePermission('users.view'), async (req, res) => {
    try {
        const { id } = req.params;
        const userQuery = `
      SELECT u.*, 
...
app.put('/users/:id', authenticateToken, requirePermission('users.edit'), async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/projects', authenticateToken, requirePermission('projects.view'), async (req, res) => {
    try {
...
app.post('/projects', authenticateToken, requirePermission('projects.create'), async (req, res) => {
    try {
        const { title, description, clientId, designerId, budget, startDate, endDate, city, address, areaSqft, propertyType, leadId } = req.body;
    ...
    `, [
        ...
        if (leadId) {
            await pool.query('UPDATE leads SET status = $1, converted_to_project = $2 WHERE id = $3', ['converted', newProject.id, leadId]);
        }
...
app.get('/projects/:id', authenticateToken, requirePermission('projects.view'), async (req, res) => {
    try {
        const { id } = req.params;
        const projectQuery = `
      SELECT 
...
app.delete('/projects/:id', authenticateToken, requirePermission('projects.manage'), async (req, res) => {
    try {
        const { id } = req.params;
...
app.post('/quotations', authenticateToken, requirePermission('finance.create'), async (req, res) => {
    try {
      ...
      INSERT INTO quotations (quotation_number, client_id, project_id, title, subtotal, total_amount, valid_until, prepared_by, status)
      VALUES ($1, $2, $3, $4, $5, $6, $7, $8, 'draft') RETURNING *
    `, [quotationNumber, clientId, projectId, title, totalAmount, totalAmount, validUntil, req.user.id]);
...
// ==================== LEAD MANAGEMENT ENDPOINTS ====================
app.get('/leads', authenticateToken, requirePermission('leads.view'), async (req, res) => {
    try {
...
app.post('/leads', async (req, res) => {
    try {
        const { source, firstName, lastName, email, phone, city, budgetMin, budgetMax, projectType, propertyType, timeline, description } = req.body;
        ...
        let score = 0;
        if (budgetMin && budgetMin > 500000)
            score += 30;
        else if (budgetMin && budgetMin > 200000)
            score += 20;
        else if (budgetMin)
            score += 10;
        if (timeline === 'immediate')
            ...
            score += 10;
        if (projectType === 'full_home')
            score += 20;
        else if (projectType === 'multiple_rooms')
            score += 15;
        else if (projectType === 'single_room')
            score += 10;
...
app.get('/leads/:id', authenticateToken, requirePermission('leads.view'), async (req, res) => {
    try {
        const { id } = req.params;
        const leadQuery = `
      SELECT 
...
app.post('/leads/:id/assign', authenticateToken, requirePermission('leads.assign'), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updateResult = await pool.query('UPDATE leads SET assigned_to = $1, updated_at = NOW() WHERE id = $2 RETURNING *', [assignedTo, id]);
        if (updateResult.rows.length === 0) {
            return res.status(404).json({ error: 'Lead not found' });
        }
...
app.post('/leads/:id/convert', authenticateToken, requirePermission('leads.convert'), async (req, res) => {
    try {
        const { id } = req.params;
    ...
    `, [
        ...
        const newProject = projectResult.rows[0];
        // Update lead status
        await pool.query('UPDATE leads SET status = $1, converted_to_project = $2, updated_at = NOW() WHERE id = $3', ['converted', newProject.id, id]);
        res.json({
...
app.post('/materials', authenticateToken, requirePermission('vendors.manage'), async (req, res) => {
    try {
        const { vendorId, name, category, subcategory, brand, model, description, unit, price, discountedPrice, stockQuantity, minOrderQuantity, leadTimeDays, images, specifications } = req.body;
...
app.put('/materials/:id', authenticateToken, requirePermission('vendors.manage'), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, category, subcategory, brand, model, description, unit, price, discountedPrice, stockQuantity, minOrderQuantity, leadTimeDays, images, specifications, isActive } = req.body;
        const updateResult = await pool.query(`
...
app.get('/vendors', authenticateToken, requirePermission('vendors.view'), async (req, res) => {
    try {
...
app.post('/vendors', authenticateToken, requirePermission('vendors.manage'), async (req, res) => {
    try {
        const { userId, companyName, businessType, gstNumber, panNumber, address, city, state, pincode } = req.body;
...
app.get('/vendors/:id', authenticateToken, requirePermission('vendors.view'), async (req, res) => {
    try {
        const { id } = req.params;
        const vendorQuery = `
      SELECT 
...
app.get('/vendors/:id/materials', authenticateToken, async (req, res) => {
    try {
        const { id } = req.params;
...
app.get('/analytics/leads', authenticateToken, requirePermission('analytics.view'), async (req, res) => {
    try {
...
app.get('/analytics/projects', authenticateToken, requirePermission('analytics.view'), async (req, res) => {
    try {
        const statusStats = await pool.query(`
      SELECT status, COUNT(*) as count 
      FROM projects 
...
app.get('/rbac/user-permissions', authenticateToken, async (req, res) => {
    ...
    `;
        ...
        res.json({
            permissions: result.rows[0]?.permissions?.filter((p) => p) || []
        });
    }
    catch (error) {
